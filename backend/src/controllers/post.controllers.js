...
const addNewPost = asyncHandler(async (req,res) => {
    // get the data
    const {departmentId, title, description, latitude, longitude, address} = req.body
    ...
    if(!localImageFiles) throw new ApiError(500, "Error occurred while uploading images")

    if(!mongoose.isValidObjectId(departmentId)) throw new ApiError("Invalid department ID")
    
    if(
        [title, description, address].some(field => field === "") || !latitude || !longitude
    ...
    if(!imageUploadResponse) throw new ApiError(500, "An unexpected Error occurred while uploading images")
    ...
    if(!response) throw new ApiError(500, "An unexpected error occurred while creating a post")

    res
    .status(201)
    .json(
        new ApiResponse(201, response, "Post added successfully")
    )
})
...
const getPostsByLocation = asyncHandler(async (req,res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200, posts, "Posts fethced successfully")
    )
...
const getPostById = asyncHandler(async (req,res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200, response, "Post fetched successfully")
    )
})
...
const deletePost = asyncHandler(async (req,res) => {
    ...
    if(!cloudinaryDeleteResponse) throw new ApiError(500, "An unexpected error occurred while deleting images from cloudinary")
...
const updatePostStatus = asyncHandler(async (req,res) => {
    ...
    if(!mongoose.isValidObjectId(postId)) throw new ApiError(400, "Post id is Invalid")
    ...
    if(!postToUpdate) throw new ApiError(500, "Unexprected error occurred while fetching post details")

    // check if the user role is citizen and user id's matches or not
    if(user.role === 'citizen' && postToUpdate.userId !== user._id) throw new ApiError(400, "User is not authorized to update status of current post")

    // check if the user role is authority and post is under their department or not
    if(user.role === 'authority' && postToUpdate.departmentId !== user.departmentId) throw new ApiError(400, "Current Post does not belong under your department")
    ...
    if(!newComment) throw new ApiError(500, "An error occurred while updating post")
