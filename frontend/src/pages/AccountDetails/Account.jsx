...
function Account() {
  ...
  const getAddress = async () => {
    try{
        const response = await getAddressFromCoordinates({lat: user.location.coordinates[1], lng: user.location.coordinates[0]});
        setAddress(response)
    } catch(error) {
        ToasterNotification({
            type: "warning",
              ...
              <div className="w-full p-4 gap flex flex-col mt-3 items-center gap-5">
                  ...
                  <div className="flex w-full flex-col gap-3 items-center justify-center">
                      <div className="w-24 h-24 rounded-full overflow-hidden  ">
                          ...
                          :
                            <img src={previewNewImageUrl} alt="profile image" className="object-contain"/>
                          }
                      </div>
                      <input 
                        type="file" 
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        {previewNewImageUrl !== "" && <button onClick={handleRemoveImage} className="bg-red-100 dark:bg-zinc-800 text-red-500 p-2 text-sm rounded-lg">Cancel</button>}
                      </div>
                  </div>
                  ...
                  <div className="w-full flex gap-2 justify-between items-center">
                      ...
                      <div className="flex flex-col gap-[1px]">
                        ...
                        {...register("name", {
                          minLength: {value: 5, message: "Name must be atleast 5 characters long"},
                          maxLength: {value: 30, message: "Name must be atmost 30 characters long"},
                  ...
                  {/* Address field  */}
                  <div className="w-full flex gap-2 justify-between items-center">
                      <label htmlFor="role" className=" text-zinc-400" >Address</label>
                      <input 
                      type="text" 
                      id="role"
                      disabled
                  ...
                  {/* Submit button  */}
                  <button type="submit" className="bg-blue-700 shadow-inner px-3 py-2 text-white rounded-lg">
                      Update Details
                  </button>
              </div>
            </form>
